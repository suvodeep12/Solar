...
- CameraRig: selection flies the camera via `targetDist` + `flyTo` flag — a per-frame `dist` lerp toward the live distance silently ate the zoom target (verify with a click, not just centering); manual pan/zoom lerps `offset`/`dist` separately
- Click reliability: `uiStore.select()` bumps `focusTick` on every call so re-selecting the SAME body re-files the camera (zustand same-value sets otherwise skip `CameraRig`'s `[selectedId]` effect — deps are `[selectedId, focusTick]`); every body has an invisible oversized hit sphere (`max(0.3, radiusScene*2)`; sun `max(0.6, 1.5×)`, opacity-0 basic material, depthWrite false) so 3px bodies are clickable at overview without near-miss deselects (`onPointerMissed` → `select(null)` fires on any missed click). The Sun is a full selectable body: `StarSpec` (kind `'star'` — a SINGLETON literal, so `kind === 'star'` narrows the `SelectableSpec` union; a wide `BodyKind` discriminant there makes TS type-guard false branches misbehave) is in `ALL_BODIES`, CameraRig flies to origin with `sunRadiusScene()*12`, InfoPanel renders a star layout (no orbit rows). Sun label clickable like planet labels. Moons are NOT selectable (MoonSpec has no BodySpec) — deliberate
